...
function AdSlide({ ad }: { ad: AdData }) {
  ...
  return (
    <div style={{ position: "relative", width: 360, height: 640, overflow: "hidden", backgroundColor: "#0a0806" }}>
      ...
      {/* ── FOOTER — WhatsApp contact only ── */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 96, backgroundColor: "rgba(5,3,2,0.93)", display: "flex", alignItems: "center", justifyContent: "center", gap: 16, zIndex: 3 }}>
        <svg width={32} height={32} viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
          <path fill={B.orange} d={WA1} />
          <path fill={B.orange} d={WA2} />
        </svg>
        <div>
          <div style={{ fontFamily: FB, fontSize: 20, fontWeight: 700, color: "#fff", letterSpacing: "0.04em", lineHeight: 1, whiteSpace: "nowrap" as const }}>{ad.phone}</div>
          <div style={{ fontFamily: FB, fontSize: 7.5, color: B.orange, letterSpacing: "0.20em", textTransform: "uppercase" as const, marginTop: 6, whiteSpace: "nowrap" as const }}>WHATSAPP</div>
        </div>
      </div>
    </div>
  );
